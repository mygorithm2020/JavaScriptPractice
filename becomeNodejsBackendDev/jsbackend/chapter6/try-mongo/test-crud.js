...
const url = "mongodb+srv://mygoTest:test1234@cluster0.e3nsvf7.mongodb.net/?retryWrites=true&w=majority";
...
async function main() {
  try {
    ...
    // Document 하나 추가
    await collection.insertOne({ name: 'col', age: 30, sex : "M"});
    console.log('document 추가 완료');

    // Document 찾기 
    const documents = await collection.find({ name: 'col' }).toArray();
    console.log('찾은 document:', documents);

    // Document 갱신하기
    await collection.updateOne({ name: 'col' }, { $set: { age: 31 } });
